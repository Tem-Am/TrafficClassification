...
def Generator(opts):
    ...
    # v0: 49 features defined in the next line
    ...
    # v1: 17 features defined in the next line
    col = ['file_name','Malware_label','type','sa','da','sp','dp','pr','tls_scs','tls_ext_server_name','tls_c_key_length', 'ttlout', 'ttlin']
    
    # # v0 and v1: 400 feature defined in the next for loop
    # for i in range(20):
    #     for j in range(20):
    #         splt_str = 'splt_' + str(i) + '_' + str(j) 
    #         col.append(splt_str)

    # v0 and v1: 257 features defined in the next for loop + entropy
    for i in range(256):
        dist_str = 'dist_' + str(i)
        col.append(dist_str)
    col.append('entropy')

    # v1: 150 packet features (50*3)
    for i in range(packet_num):
        packet_str_b = 'packet_' + str(i) + '_b'
        packet_str_ipt = 'packet_' + str(i) + '_ipt'
        packet_str_dir = 'packet_' + str(i) + '_dir'
        col.append(packet_str_b)
        col.append(packet_str_ipt)
        col.append(packet_str_dir)
    
    # total features of v0: 705
    # total features of v1: 824
   ...
   # df = pd.DataFrame(columns = col)
    ...
    for i in range(0,len(data)):
        ...
        if(opts.is_malware == 1):
            df.loc[i,'Malware_label'] = 1
        else:
            df.loc[i,'Malware_label'] = 0
        if(i%100 == 0):
    ...
    print("The size of the data frame (df) is: ", df.shape)
...
def packets(data, i, df):
    if(data[i].__contains__("packets")):
        packets = data[i]['packets']
        if(len(packets) != 0):
            k = 0
            for k in range(packet_num):   # only consider the first 50 packets (including both directions) instead of range(len(packets))
                packet_str_b = 'packet_' + str(k) + '_b'
                packet_str_ipt = 'packet_' + str(k) + '_ipt'
                packet_str_dir = 'packet_' + str(k) + '_dir'
                df.loc[i, packet_str_b] = 0     # no packet
                df.loc[i, packet_str_ipt] = 999 # no packet
                df.loc[i, packet_str_dir] = 3   # no packet
                if (k < len(packets)):
                    if(packets[k].__contains__('b')):
                        df.loc[i, packet_str_b] = packets[k]['b']
                    if(packets[k].__contains__('ipt')):
                        df.loc[i, packet_str_ipt] = packets[k]['ipt']
                    if(packets[k].__contains__('dir')):
                        if(packets[k]['dir'] == '>'):
                            df.loc[i, packet_str_dir] = 0   # > direction
                        else:
                            df.loc[i, packet_str_dir] = 1   # < direction
